...
- **Stress Level을 1~5점으로 예측하는 것에 있어서 이는 정량, 정성 데이터가 혼합된 변수들이 예측하는 심리적 지표이며 변수 간 상호작용이 존재할 수 있고 비선형 분포를 따르기 때문에 개별 트리가 독립적으로 학습되는 Random Forest보다 이전 모델의 오차를 학습하며 변수 간 비선형 결합을 반복적으로 보완하는 XGBoost를 이용하는 것이 더 적합하기 때문에, XGBoost가 약간이나마 더 높은 정확도를 보인 것으로 예상됨.**
...
| 이름 | 프로젝트 내 역할 |
| :-- | :-- |
| 손영훈&nbsp; | 전체 분석의 기반이 되는 주요 코드 구현을 담당하였으며,<br> 각 피처(변수)별로 모델이 어떻게 반응하는지에 대한 결과 분석을 수행하였습니다. |
| 이수호&nbsp; | 전처리된 데이터를 바탕으로 주요 특성을 분석하였고,<br> 프로젝트의 흐름과 결과를 정리한 블로그를 작성하였으며, 발표 영상 녹화를 통해 시각적 전달에도 기여하였습니다. |
| 박승우&nbsp; | 다양한 학습 기법을 비교 분석하여 모델별 성능 차이를 정리하였고,<br> 학습 결과를 표와 그래프로 시각화하여 분석 내용을 한눈에 볼 수 있도록 구성하였습니다. |
| 신동욱&nbsp; | 프로젝트의 최종 결과물을 바탕으로 종합적인 코드 정리를 수행하였으며,<br> 이를 바탕으로 결론을 도출하고 전체 분석 내용을 명확하게 정리하였습니다. |
